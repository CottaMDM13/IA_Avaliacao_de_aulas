    ...
    if request.method == "POST":
        ...
        try:
            if 'videos' not in request.files:
                ...
                return render_template("error.html", error="Campo 'videos' ausente no formulário."), 400
            video_files = request.files.getlist("videos")
            if not video_files or all(not f.filename for f in video_files):
                logger.error("Nenhum arquivo válido enviado")
                return render_template("error.html", error="Nenhum arquivo válido enviado."), 400
            ...
            for i, video_file in enumerate(video_files):
                ...
                if not video_file.filename:
                    ...
                    return render_template("error.html", error="Um ou mais arquivos estão sem nome."), 400
                
                if not video_file.filename.lower().endswith('.mp4'):
                    logger.error(f"Arquivo inválido: {video_file.filename}")
                    return render_template("error.html", error=f"Arquivo {video_file.filename} inválido. Apenas .mp4 é aceito."), 400
                ...
                if file_size > 1024 * 1024 * 1024:  # 1GB
                    ...
                    return render_template("error.html", error=f"Arquivo {video_file.filename} excede o tamanho máximo de 1GB."), 413
                ...
                if mime.from_buffer(file_content) != 'video/mp4':
                    ...
                    return render_template("error.html", error=f"Arquivo {video_file.filename} não é um vídeo MP4 válido."), 400
                video_file.seek(0)
                ...
                try:
                    ...
                    if not os.path.exists(audio_path):
                        ...
                        return render_template("error.html", error=f"Falha na extração de áudio para {video_file.filename}."), 500
                    ...
                    logger.debug(f"Progresso: {i+1}/{total_files}")
                    
                except Exception as e:
                    logger.error(f"Erro ao processar {video_file.filename}: {str(e)}")
                    return render_template("error.html", error=f"Erro ao processar {video_file.filename}: {str(e)}"), 500
            ...
            return render_template("error.html", error="O vídeo é muito grande. O tamanho máximo é 1GB."), 413
        except Exception as e:
            logger.error(f"Erro inesperado ao processar upload: {str(e)}")
            return render_template("error.html", error=f"Erro inesperado: {str(e)}"), 500
...
def delete_report(report_id):
    try:
        ...
        if "paths" not in config or "output_reports" not in config["paths"]:
            ...
            return render_template("error.html", error="Configuração inválida: caminho 'output_reports' não definido."), 500
        output_dir = config["paths"]["output_reports"]
        db_path = f"{output_dir}/reports.db"
        if not os.path.exists(output_dir):
            logger.error(f"Diretório de relatórios não encontrado: {output_dir}")
            return render_template("error.html", error=f"Diretório de relatórios não encontrado: {output_dir}"), 500
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        ...
        return render_template("error.html", error=f"Erro ao excluir relatório: {str(e)}"), 500
